...
.PHONY: all build build-linux build-init build-init-linux test test-integration vet lint lint-fix fmt fmt-check vuln check clean
...
build-init:
	CGO_ENABLED=0 go build -o $(INIT_BIN) ./cmd/shard-init

# The guest arch must match the box that runs the sandbox, so this one ships beside shard.
build-init-linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o $(INIT_BIN)-linux-amd64 ./cmd/shard-init
