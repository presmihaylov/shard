...
func Run(ctx context.Context, cfg Config) error {
	...
	return New(cfg.Root, cfg.Out, apiTask{deps: d, lifecycle: life, process: self}, proxyTask{deps: d}, egressLogRotation{deps: d}, egressLogTailer{deps: d}, oomRestart{deps: d, lifecycle: life, interval: oomInterval}, healthCheck{deps: d, lifecycle: life, interval: healthInterval}).WithReconciler(reconciler{deps: d, lifecycle: life}).Run(ctx)
}
