...
// Binary is the OCI runtime the provider drives, on PATH wherever the runc package is installed.
const Binary = "runc"
...
const (
	// pollInterval paces every wait here. runc state is a process spawn, so it is not free.
...
func New(runner *runccli.Runner, bundles *bundle.Service, dirs StateDirs) (*Provider, error) {
...
func (p *Provider) Create(ctx context.Context, spec models.SandboxSpec) error {
	...
	// A rootfs that stands while runc holds nothing may still be a live sandbox's, so never build over it.
...
func (p *Provider) create(ctx context.Context, spec models.SandboxSpec, b bundle.Bundle) (err error) {
	...
	if err := p.runner.Create(ctx, spec.ID, runccli.CreateOptions{Bundle: b.Dir, Stdout: out, Stderr: out, Stdin: exit}); err != nil {
		return err
	}
...
func (p *Provider) Stop(ctx context.Context, id string, grace time.Duration) error {
	...
	// A frozen cgroup delivers no signal. Nothing of shard's freezes a runc sandbox, so this is
	...
	// runc still holds a sandbox it has stopped, so the status read above is what owns the mount.
	return p.unmount(id, status.Exists)
}
...
// Remove deletes runc's own state. The record and the state directory belong to the repository.
func (p *Provider) Remove(ctx context.Context, id string) error {
	// runc delete --force exits 0 for an id it never held, so only a status read says who owns the rootfs.
	status, err := p.Status(ctx, id)
	if err != nil {
		return err
	}
...
// unmount drops the merged view and the disk under it. The upper layer stays, which is what a later create reads back.
// held says whether runc knew the sandbox, because only that answers who owns the rootfs.
...
// orphaned refuses a rootfs that stands while runc holds nothing: something deleted the
...
func orphaned(b bundle.Bundle, id string, held bool) error {
	...
	return fmt.Errorf("runc does not hold sandbox %s but its rootfs is still mounted at %s", id, b.RootFS)
}

// gone reports whether a signal failed because the sandbox had already ended, which is what a stop wants.
func gone(err error) bool {
	return errors.Is(err, runccli.ErrNotRunning) || errors.Is(err, runccli.ErrNotFound)
}
...
func (p *Provider) Exec(ctx context.Context, id string, spec models.ExecSpec) (models.ExitStatus, error) {
	...
	// runc exec reports its own startup failures as the command's exit 1, so an exit code
...
func stateOf(status runccli.Status) models.State {
	switch status {
	case runccli.StatusRunning:
		return models.StateRunning
	case runccli.StatusPaused:
		return models.StatePaused
	case runccli.StatusStopped:
