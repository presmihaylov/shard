...
func newHarness(t *testing.T) *harness {
	...
	h.provider, err = runc.New(runner, bundles, h.stateDir)
	if err != nil {
	...
	h.open = func() (models.Provider, error) { return runc.New(runner, bundles, h.stateDir) }
...
// No network: runc makes a namespace of its own, which is all the suite needs.
func (h *harness) newSpec(t *testing.T, entrypoint ...string) models.SandboxSpec {
	t.Helper()

	id := fmt.Sprintf("shard-222-%d", h.next.Add(1))
	dir := t.TempDir()
...
func requireRunc(t *testing.T) {
	t.Helper()

	if os.Geteuid() != 0 {
		t.Skip("runc needs root")
	}
	if _, err := exec.LookPath(runc.Binary); err != nil {
		t.Skip("no runc on this host")
	}
}
...
func TestMain(m *testing.M) {
	if err := hostclean.Refuse(stateRoots()...); err != nil {
		fmt.Fprintln(os.Stderr, "runc integration tests:", err)
		os.Exit(1)
	}
	teardownOnSignal()

	root, err := os.MkdirTemp("", "shard-222-images")
	if err != nil {
...
func stateRoots() []string { return []string{filepath.Join(os.TempDir(), "shard-222-images")} }
...
func sweepCgroups() {
	left, err := filepath.Glob(filepath.Join(cgroup.Root, bundle.CgroupsPath("shard-222-*")))
	if err != nil {
