...
|---|---|---|
| GitHub Linux runner (`ci.yml`) | `make check`: build, vet, lint, unit tests, the linux cross-compile | Every PR. The Linux build has no cgo, so it runs anywhere. |
...
`make check` stays Linux-only and is the gate CI enforces. The macOS job is the same gate on the
other platform: a change to `pkg/vz`, `pkg/vzshim`, `cmd/shard-vz-shim` or
...
Record the Mac, the macOS version and the head that passed in the release notes.

## Cutting a release

Push a tag of the form `v*` on `main`. `release.yml` builds `shard-linux-amd64` and
`shard-init-linux-amd64` on a Linux runner, `shard-darwin-arm64` and `shard-darwin-amd64` on a
macOS runner, and publishes them with a `SHA256SUMS` under a GitHub release named after the tag.
`shard --version` reports the tag.
