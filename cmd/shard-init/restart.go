...
func parseRestart(policy, file string, retries int, backoff time.Duration) (restartPolicy, error) {
	parsed := restartPolicy{policy: models.RestartPolicy(policy), retries: retries, backoff: backoff, file: file}
	...
	if retries < 1 {
		return restartPolicy{}, fmt.Errorf("-retries must be at least 1, got %d", retries)
	}
	if backoff <= 0 {
