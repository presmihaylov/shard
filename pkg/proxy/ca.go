...
// mintMu keeps two callers in the daemon from minting two CAs; the daemon lock keeps a second daemon off the root.
var mintMu sync.Mutex

// LoadCA reads the CA under dir and mints one when there is none, so every caller gets the same one.
