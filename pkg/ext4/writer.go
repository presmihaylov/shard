// Forked from github.com/Microsoft/hcsshim ext4/internal/compactext4 at v0.15.0-rc.1 (MIT, see NOTICE).
...
// Writer writes a compact ext4 file system, one that Grow can extend later; paths use '/'.
type Writer struct {
...
const (
	...
	maxFileSize        = 128 << 30 // 128GB file size maximum for now
...
var errExceededMaxSize = fmt.Errorf("disk exceeded maximum size of %d bytes", MaxDiskSize)

var directoryEntrySize = binary.Size(DirectoryEntry{})
...
func (w *Writer) getInode(i InodeNumber) *inode {
...
func (w *Writer) writeXattrs(inode *inode, state *xattrState) error {
	...
	if len(state.inode) != 0 {
		...
		binary.LittleEndian.PutUint32(inode.XattrInline[0:], XAttrHeaderMagic) // Magic
		putXattrs(state.inode, inode.XattrInline[4:], 0)
	}

	// An inode that had an xattr block keeps one, even when it is now empty.
	if len(state.block) != 0 || inode.XattrBlock != 0 {
		...
		binary.LittleEndian.PutUint32(b[0:], XAttrHeaderMagic) // Magic
...
func (w *Writer) write(b []byte) (int, error) {
	...
	if w.pos+int64(len(b)) > MaxDiskSize {
		w.err = errExceededMaxSize
		return 0, w.err
	}
	n, err := w.bw.Write(b)
	w.pos += int64(n)
...
func (w *Writer) makeInode(f *File, node *inode) (*inode, error) {
	...
	case S_IFLNK:
		...
		if size <= smallSymlinkSize {
			// A short symlink lives in the inode's block field, with no inline data flag.
			node.Data = make([]byte, len(f.Linkname))
			copy(node.Data, f.Linkname)
		}
	case S_IFDIR, S_IFIFO, S_IFSOCK, S_IFCHR, S_IFBLK:
	default:
	...
	node.Size = size
	if typ == S_IFLNK && size > smallSymlinkSize {
...
func (w *Writer) Create(name string, f *File) error {
	...
	if child.Mode&TypeMask == S_IFREG {
		w.startInode(name, child, f.Size)
	}
	return nil
}

// Link adds a hard link, to a symlink itself rather than its target, as containerd does.
func (w *Writer) Link(oldname, newname string) error {
	...
	switch oldfile.Mode & TypeMask {
	case S_IFDIR:
		return fmt.Errorf("%s: link target cannot be a directory: %s", newname, oldname)
	}

	if existing != oldfile && oldfile.LinkCount >= MaxLinks {
...
func fillExtents(hdr *ExtentHeader, extents []ExtentLeafNode, startBlock, offset, inodeSize uint32) {
	*hdr = ExtentHeader{
	...
	for i := range extents {
		...
		length := min(inodeSize-block, maxBlocksPerExtent)
		start := startBlock + block
		extents[i] = ExtentLeafNode{
			Block:    block,
...
func modeToFileType(mode uint16) FileType {
...
func (w *Writer) writeDirectory(dir, parent *inode) error {
	...
	writeEntry := func(ino InodeNumber, name string) error {
...
func (w *Writer) writeInodeTable(tableSize uint32) error {
	...
	for _, inode := range w.inodes {
		if inode != nil {
			...
			switch inode.Mode & TypeMask {
			case S_IFDIR, S_IFREG, S_IFLNK:
				copy(binode.Block[:], inode.Data)
...
// NewWriter returns a Writer that writes an ext4 file system to f.
func NewWriter(f io.ReadWriteSeeker) *Writer {
	return &Writer{
		f:  f,
...
func (w *Writer) init() error {
	...
	w.seekBlock(1 + gdBlocks)
	w.initialized = true

	// The lost+found directory is required to exist for e2fsck to pass.
	if err := w.Create("lost+found", &File{Mode: S_IFDIR | 0700}); err != nil {
		return err
	}
	return w.err
}

func groupCount(blocks uint32, inodes uint32) uint32 {
	...
	// Enough groups for every inode, even when the data is small.
...
func (w *Writer) Close() error {
	...
	groups := groupCount(inodeTableOffset, uint32(len(w.inodes)))
	...
	if usedGdBlocks > gdBlocks {
		return errExceededMaxSize
	}

	gds := make([]GroupDescriptor, gdBlocks*groupsPerDescriptorBlock)
	...
	for g := range groups {
		...
		if g == 0 {
			// e2fsck wants the unused descriptor blocks free; Grow claims them as it needs them.
			for j := 1 + usedGdBlocks; j < 1+gdBlocks; j++ {
				b[j/8] &^= 1 << (j % 8)
				usedBlockCount--
			}
		}
		if g == groups-1 && diskSize%blocksPerGroup != 0 {
			// Blocks past the end of the disk are marked allocated; Grow gives them back.
		...
		// Inode bitmap
		for j := range inodesPerGroup {
			ino := InodeNumber(1 + g*inodesPerGroup + j)
			inode := w.getInode(ino)
			if ino < inodeFirst || inode != nil {
				b[BlockSize+j/8] |= 1 << (j % 8)
				usedInodeCount++
			}
			if inode != nil && inode.Mode&TypeMask == S_IFDIR {
				dirCount++
			}
		}
		setInodePadding(b[BlockSize:])
		_, err := w.write(b[:])
		if err != nil {
			return err
		}
	...
	sb := &SuperBlock{
		...
		InodeSize:          inodeSize,
		FeatureCompat:      CompatSparseSuper2 | CompatExtAttr,
		FeatureIncompat:    IncompatFiletype | IncompatExtents | IncompatFlexBg,
		FeatureRoCompat:    RoCompatLargeFile | RoCompatHugeFile | RoCompatExtraIsize,
...
func setInodePadding(bitmap []byte) {
